...
def process_file(filename, verbose=True, do_overwrite=True):
    ...
    # open the root file and grab the trees
    root_file = TFile(filename)
    tree = []
    n_entries = []
    nonempty_trees = [] ## this list saves the trees that are not empty
    for i in xrange(16):
        tree.append(root_file.Get("tree%i" % i))
        try:
            n_entries.append(tree[i].GetEntries())
        except AttributeError:
            print "==> problem accessing tree -- skipping this file"
            return 0
        if n_entries[i] > 0:
            nonempty_trees.append(i)

    n_channels = len(nonempty_trees) ## def of n_channels
    if n_channels == 0: ## if all trees are empty
        print "==> All trees are empty! -- skipping this file"
        return 0

    # read global parameters from first entry of nonempty tree:
    first_tree = nonempty_trees[0]
    tree[first_tree].GetEntry(0)
    is_external = tree[first_tree].is_external
    n_baseline_samples_to_use = tree[first_tree].wfm_delay / 4
    sampling_freq = tree[first_tree].sampling_freq
...
#        hist.SetLineColor(TColor.kBlue)
    ...
    is_2Vinput = array('I', [1]*n_channels) # unsigned int
