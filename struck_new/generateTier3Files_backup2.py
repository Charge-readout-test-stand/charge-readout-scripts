...
def process_file(filename, verbose=True, do_overwrite=True):
    ...
    for i in xrange(16): 
        ...
        try:
            tree[i].GetEntries() # if tree is empty
    ...
    if not tree[0].GetEntry(0).is_external:
        return 0 #FIXME--now only works for externally triggered events
    ...
    out_tree.Branch('channel', channels, 'channel/b')
    ...
    for i in xrange(n_channels) ## set the global parameter values
        ...
        is_pospolarity = tree[i].is_pospolarity
    ...
    for (i, channel) in enumerate channels:
        calibration[i] = struck_analysis_parameters[channel]
    ...
    timestampDouble = array('I', [0]) # unsigned int
...
#        trigger_time[0] = 200/sampling_freq_Hz*1e6
    ...
    for i_entry in xrange(min(n_entries)):
       ...
       # print "pmt_threshold:", pmt_threshold[0]
        ...
        pz_p1 = array('d', [0]*n_channels) # double
        out_tree.Branch('pz_p1', pz_p1, 'pz_p1[%i]/D' % n_channels)
        pz_p1_err = array('d', [0]*n_channels) # double
        out_tree.Branch('pz_p1_err', pz_p1_err, 'pz_p1_err[%i]/D' % n_channels)
