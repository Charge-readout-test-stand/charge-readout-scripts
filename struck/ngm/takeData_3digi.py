...
def takeData(doLoop=False, n_hours=10.0):
  ...
  #file_suffix = "_cathode_pulser_10dB_XChannels_" # this gets appended to the file name
  file_suffix  = "_trig_test_VME_"  


  runDuration = 2*10 # seconds
  ...
  #trig_string = '1' # ch 0
  #trig_string = '1000000000000000' # ch 15
  ...
  print "hex trigger output", hex(nimtrigoutput) # this is what the gui takes
  ...
  while hours_elapsed < n_hours:
      try:
          ...
          #For running with 3
          sis.GetConfiguration().GetSlotParameters().SetParameterS("IPaddr",0,"192.168.2.102") # 97
          sis.GetConfiguration().GetSlotParameters().SetParameterS("IPaddr",1,"192.168.2.100") # 98
          #sis.GetConfiguration().GetSlotParameters().SetParameterS("IPaddr",2,"192.168.2.101") # DT
          ...
          for icard in xrange(2): # loop over cards:
            ...
            #New for 3 digi
            if icard == 0: sis0.nimtrigoutput = nimtrigoutput
