    ...
    for i_entry in xrange(tree.GetEntries()):
        ...
        if tree.channel == channel:
            ...
            for i in range(mawlen):
                maw[i] = - np.sum(wfm[i:i+peaking_time]) + np.sum(wfm[i+peaking_time+gap_time:i+2*peaking_time+gap_time])
                if i >= offset and i < offset+100:
                    mawplot.SetPoint(i, i-offset, maw[i])
            ...
            if val != 'q':
                ...
                canvas.Update()
                val = raw_input("Enter p to print, q to stop drawing: ")
