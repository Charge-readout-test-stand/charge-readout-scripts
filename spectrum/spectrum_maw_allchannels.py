...
def process_file(filename):
    ...
    for i_entry in xrange(tree.GetEntries()):
        tree.GetEntry(i_entry)
        if tree.channel not in spectrums:
            spectrums[tree.channel] = TH1D("spectrum", "Spectrum", 500, 0.0, 500.0)
        ...
        spectrums[tree.channel].Fill(np.amax(maw))


    canvas.Clear()

    for channel in spectrums:
        spectrums[channel].Draw()
        canvas.Update()
